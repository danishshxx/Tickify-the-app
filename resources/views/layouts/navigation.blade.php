<nav x-data="{ open: false }" class="bg-black/90 backdrop-blur-md border-b border-gray-800 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}">
                        <x-application-logo class="block h-14 w-auto fill-current text-gray-200" />
                    </a>
                </div>

                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if(Auth::user()->role === 'admin')
                        <x-nav-link :href="route('admin.dashboard')" :active="request()->routeIs('admin.dashboard')" class="text-white hover:text-purple-400">
                            {{ __('Admin Panel') }}
                        </x-nav-link>
                        <x-nav-link :href="route('admin.transactions')" :active="request()->routeIs('admin.transactions')" class="text-white hover:text-purple-400">
                            {{ __('Validasi Pembayaran') }}
                        </x-nav-link>
                    @else
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-white hover:text-purple-400">
                            {{ __('Tiket Saya') }}
                        </x-nav-link>
                    @endif
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')" class="bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-white">
                            {{ __('Profile') }}
                        </x-dropdown-link>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                    onclick="event.preventDefault();
                                                this.closest('form').submit();" class="bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-white">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-900 border-t border-gray-800">
        <div class="pt-2 pb-3 space-y-1">
            @if(Auth::user()->role === 'admin')
                ...
                <x-responsive-nav-link :href="route('admin.transactions')" :active="request()->routeIs('admin.transactions')" class="text-white">
                    {{ __('Validasi Pembayaran') }}
                </x-responsive-nav-link>
            @else
